...
{
    private Mock<IMovieService> _managerMock;

    [TestInitialize]
    public void Setup()
    {
        _managerMock = new Mock<IMovieService>(MockBehavior.Strict);
    }
    ...
    public void GetExistingMovieReturnsAsExpected()
    {
        ...
        _managerMock.Setup(manager => manager.GetMovieById(It.IsAny<int>())).Returns(movie);
    ...
    public void GetNonExistingMovieReturnsNotFound()
    {
        ...
        _managerMock.Setup(manager => manager.GetMovieById(It.IsAny<int>()))
            .Throws(exception);
        var controller = new MovieController(_managerMock.Object);
